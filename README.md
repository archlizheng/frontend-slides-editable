...
- **双构建层**：`STYLE_PRESETS.md` 中 **46** 套预设与 `examples/generated/presets/*.html` **一一对应**。其中 19 套 legacy preview 由 `scripts/build-preset-decks.py` 生成；34 套 ported preview 由 `scripts/build-template-port-decks.py` 从本地 `beautiful-html-templates` 真实模板生成，并覆盖同名 legacy 输出。改 legacy archetype 后跑前者；改 ported 模板或 slot runtime 后跑后者。
- **Two build layers**: all **46** presets in `STYLE_PRESETS.md` map one-to-one to `examples/generated/presets/*.html`. The 19 legacy previews come from `scripts/build-preset-decks.py`; the 34 ported previews come from `scripts/build-template-port-decks.py` and local `beautiful-html-templates` source templates, replacing same-name legacy outputs. Run the legacy builder after archetype changes; run the port builder after template-port or slot-runtime changes.
